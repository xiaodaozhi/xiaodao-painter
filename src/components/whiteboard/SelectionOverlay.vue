...
const props = defineProps<{
  stroke: Stroke
  startResize: (strokeId: string, handle: string, event: MouseEvent) => void
}>()
...
const lineHandlePositions = computed(() => {
  ...
  return [
    { name: 'line-start', x: s.x, y: s.y },
    { name: 'line-end', x: s.x + s.width, y: s.y + s.height },
  ]
})
...
function onHandleMousedown(handle: string, e: MouseEvent) {
  props.startResize(props.stroke.id, handle, e)
}
</script>
...
<template>
  <g v-if="stroke">
    ...
    <rect
      ...
      rx="1"
      style="cursor: move"
      @mousedown.stop.prevent="onHandleMousedown(h.name, $event)"
    />
